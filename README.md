...
* 网站配置详情页提供负载均衡、缓存策略、计算验证防护、恶意请求防护、地区限制和认证配置分区
...
6. 如需跨 HK、EU 等多个节点池分流，在「负载均衡」分区启用 `多节点智能解析`，点击 `+` 逐行添加节点池、权重、可选国家代码和来源网段，例如池名 `hk`、权重 `80`、国家代码 `HK,TW`、来源网段 `203.0.113.0/24`。来源网段会优先于国家代码匹配。
