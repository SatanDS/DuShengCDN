...
> [!WARNING]
> 新数据库首次启动会创建 `root` 用户。请使用 `.env` 中的 `DUSHENGCDN_INITIAL_ROOT_PASSWORD` 登录，或查看 Server 首次启动日志中的一次性随机密码；登录后请立即修改密码并移除或轮换该启动值。
...
services:
  ...
  dushengcdn:
    ...
    environment:
      ...
      DUSHENGCDN_INITIAL_ROOT_PASSWORD: ${DUSHENGCDN_INITIAL_ROOT_PASSWORD:-}
...
* 用户名：`root`
* 密码：`.env` 中的 `DUSHENGCDN_INITIAL_ROOT_PASSWORD`；如果没有配置该值，则查看 Server 首次空库启动日志中的一次性随机密码。
...
* Debian 13 `trixie` 暂无 OpenResty 官方源时，脚本会回退到 Debian `bookworm` 源；如果签名或仓库校验失败，脚本会拒绝安装未认证软件包，请手动安装 OpenResty 或使用 `--openresty-path` 指向已审阅的二进制。
