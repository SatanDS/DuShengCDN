...
首登账户为 `root`。密码优先使用 `.env` 中的 `DUSHENGCDN_INITIAL_ROOT_PASSWORD`；如果为空，Server 会把随机生成的一次性密码写入 `initial-root-password.txt`（日志只显示文件路径，不打印密码）。登录后请立即修改 root 密码，并移除或轮换该启动变量/密码文件。
...
不要把 issuer 私钥写入客户部署、README、Compose 文件、日志、shell 历史、进程参数或 release 资产；签发命令优先使用 `-private-key-file`。`DUSHENGCDN_LICENSE_ISSUER_PRIVATE_KEY` 只用于中央授权服务签发许可证，不能用于 release 资产签名；安装器和二进制 `.sig` 使用的是独立的 `DUSHENGCDN_RELEASE_SIGNING_PRIVATE_KEY`。
...
```bash
bash install-agent.sh --server-url https://cdn.example.com --agent-token-file /run/secrets/dushengcdn-agent-token --reinstall --wipe-data
```
...
把 secret 临时放在仓库外的 JSON 文件里，避免出现在 shell 历史中。工具会拒绝读取仓库内的 `-SecretsFile`：
...
正式执行前可去掉 `-Upload -Force`，只生成本地 `dist/manual-installer-assets` 并人工检查 `install-agent.sh`、`install-agent.sh.sha256`、`install-agent.sh.sig`。私钥和 token 只允许通过安全密钥管理或仓库外临时 secret 文件注入；环境变量仅作为受控 CI 兼容入口，不要在交互 shell 里手动导出私钥。临时 secret 文件执行后应立即删除。
