...
* 自建权威 DNS 与实时 GSLB 已落地 Server 控制面和 DNS Worker MVP，包含 Zone、DNS Worker Token、心跳/聚合、只读调度快照 API、UDP/TCP 53 查询回答、逐查询 GSLB 选点、管理端 DNS 查询趋势、SERVFAIL/NXDOMAIN 观测、Worker 快照一致性告警、Zone 委派检查和 Glue 提示。
...
* 自建权威 DNS 控制面、Worker 快照/心跳 API、UDP/TCP 53 查询 Worker、NS 委派检查、Glue 提示、逐查询实时 GSLB、查询趋势和快照一致性观测
