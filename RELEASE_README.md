...
DuShengCDN 是面向自托管边缘节点的 CDN 管理平台。它把站点、源站、证书、节点、缓存、访问观测、配置发布、自建权威 DNS 和节点池级配置分发放在同一个管理面板里，适合需要自己掌控边缘机器、证书和调度策略的团队。
...
- **全局版本 + 节点池配置**：一次发布保留全局版本，同时按节点池自动分发对应配置，节点列表和详情页可直接看到当前/目标配置状态，减少大范围重载和抖动。
- **边缘 Agent 托管**：Agent 在边缘节点接入面板，负责心跳、配置同步、OpenResty 应用、reload、失败回滚、缓存清理和运行状态上报。
- **移动端自适应**：面板在手机浏览器、平板和桌面端都能顺畅使用，常用节点查看、发布和授权操作都更适合直接在浏览器里完成。
...
- **DNS Worker 安全与隔离**：Worker Token 以 hash + prefix 存储，支持轮换和撤销；快照、心跳使用专用 API 限流；Zone 可分配到指定 Worker，未分配 Zone 默认不下发，可按需开启兼容模式。
...
如果已经拿到商业授权 token，可在安装时一并写入并请求激活。建议先放入权限受限文件，避免 token 出现在 shell 历史或进程参数中：

```bash
install -d -m 0700 /run/secrets
printf 'License token: ' >&2
stty -echo
IFS= read -r license_token
stty echo
printf '\n' >&2
printf '%s\n' "$license_token" | install -m 0600 /dev/stdin /run/secrets/dushengcdn-license-token
unset license_token
...
stty echo
printf '\n' >&2
printf '%s\n' "$discovery_token" | install -m 0600 /dev/stdin /run/secrets/dushengcdn-discovery-token
...
stty echo
printf '\n' >&2
printf '%s\n' "$agent_token" | install -m 0600 /dev/stdin /run/secrets/dushengcdn-agent-token
unset agent_token
...
stty echo
printf '\n' >&2
printf '%s\n' "$dns_worker_token" | install -m 0600 /dev/stdin /run/secrets/dushengcdn-dns-worker-token
