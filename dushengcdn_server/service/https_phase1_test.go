...
func TestCreateTLSCertificateAndRenderHTTPSConfig(t *testing.T) {
	...
	if !strings.Contains(result.Version.MainConfig, "include __DUSHENGCDN_ROUTE_CONFIG__;") {
	...
	if !strings.Contains(result.Version.MainConfig, "listen __DUSHENGCDN_OBSERVABILITY_LISTEN__;") {
	...
	if !strings.Contains(result.Version.MainConfig, "__DUSHENGCDN_RESOLVER_DIRECTIVE__") {
	...
	if !strings.Contains(result.Version.RenderedConfig, "__DUSHENGCDN_CERT_DIR__/") {
...
func TestPublishConfigVersionRendersRouteLevelCachePolicy(t *testing.T) {
	...
	if !strings.Contains(result.Version.MainConfig, "proxy_cache_path /var/cache/openresty/dushengcdn") {
	...
	if !strings.Contains(result.Version.RenderedConfig, "proxy_cache_bypass $dushengcdn_skip_cache;") {
		t.Fatal("expected rendered config to bypass cache when request is unsafe")
	}
	if !strings.Contains(result.Version.RenderedConfig, "proxy_no_cache $dushengcdn_skip_cache;") {
	...
	if strings.Count(result.Version.RenderedConfig, "proxy_cache dushengcdn_cache;") != 1 {
...
func TestPublishConfigVersionRendersPathContainsAllCachePolicy(t *testing.T) {
	setupServiceTestDB(t)
	if err := model.UpdateOption("OpenRestyCacheEnabled", "true"); err != nil {
		t.Fatalf("UpdateOption OpenRestyCacheEnabled failed: %v", err)
	}
	...
	_, err := CreateProxyRoute(ProxyRouteInput{
		Domain:       "emby-cover.example.com",
		OriginURL:    "https://origin.internal",
		Enabled:      true,
		CacheEnabled: true,
		CachePolicy:  proxyRouteCachePolicyPathContainsAll,
		CacheRules:   []string{"/emby/Items/", "/Images"},
	})
	if err != nil {
		t.Fatalf("CreateProxyRoute failed: %v", err)
	}
	...
	if !strings.Contains(result.Version.RenderedConfig, `if ($uri !~* "/emby/Items/")`) {
		t.Fatalf("expected rendered config to require emby path segment, got %s", result.Version.RenderedConfig)
	}
	if !strings.Contains(result.Version.RenderedConfig, `if ($uri !~* "/Images")`) {
		t.Fatalf("expected rendered config to require images path segment, got %s", result.Version.RenderedConfig)
	}
	if !strings.Contains(result.Version.SnapshotJSON, `"cache_policy":"path_contains_all"`) {
		t.Fatal("expected snapshot to include path contains all cache policy")
	}
}
...
func TestPublishConfigVersionRendersMultipleCertificatesForMultiDomainWebsite(t *testing.T) {
	...
	if strings.Count(result.Version.RenderedConfig, "ssl_certificate __DUSHENGCDN_CERT_DIR__/") != 2 {
		t.Fatalf("expected rendered config to include two ssl_certificate directives, got %s", result.Version.RenderedConfig)
	}
	if strings.Count(result.Version.RenderedConfig, "ssl_certificate_key __DUSHENGCDN_CERT_DIR__/") != 2 {
...
func TestPublishConfigVersionRendersRouteRateLimits(t *testing.T) {
	...
	if !strings.Contains(result.Version.MainConfig, "limit_conn_zone $server_name zone=dushengcdn_conn_per_server:10m;") {
		t.Fatal("expected main config to include server limit_conn_zone")
	}
	if !strings.Contains(result.Version.MainConfig, "limit_conn_zone $binary_remote_addr zone=dushengcdn_conn_per_ip:10m;") {
		t.Fatal("expected main config to include ip limit_conn_zone")
	}
	if !strings.Contains(result.Version.RenderedConfig, "limit_conn dushengcdn_conn_per_server 120;") {
		t.Fatal("expected rendered config to include per-server limit_conn")
	}
	if !strings.Contains(result.Version.RenderedConfig, "limit_conn dushengcdn_conn_per_ip 12;") {
...
func TestPreviewAndDiffConfigVersion(t *testing.T) {
	...
	if !strings.Contains(preview.MainConfig, "include __DUSHENGCDN_ROUTE_CONFIG__;") {
		t.Fatal("expected preview main config to include managed route config placeholder")
	}
	if !strings.Contains(preview.MainConfig, "log_by_lua_file __DUSHENGCDN_LUA_DIR__/log.lua;") {
...
func TestPublishConfigVersionDetectsPoWChanges(t *testing.T) {
	...
	if !strings.Contains(secondRelease.Version.MainConfig, "lua_shared_dict dushengcdn_pow_config 1m;") {
		t.Fatal("expected main config to declare shared dict for pow config")
	}
	if !strings.Contains(secondRelease.Version.MainConfig, "lua_shared_dict dushengcdn_geoip_cache 20m;") {
	...
	if !strings.Contains(secondRelease.Version.RenderedConfig, "    access_by_lua_file __DUSHENGCDN_LUA_DIR__/access.lua;\n\n    location = /.within.website/x/cmd/anubis/api/pass-challenge") {
...
func TestPublishConfigVersionRendersBasicAuthWithPoW(t *testing.T) {
	...
	if !strings.Contains(result.Version.RenderedConfig, "    access_by_lua_file __DUSHENGCDN_LUA_DIR__/access.lua;") {
...
func TestPublishConfigVersionRendersRegionRestriction(t *testing.T) {
	...
	if !strings.Contains(result.Version.MainConfig, "lua_shared_dict dushengcdn_region_config 1m;") {
