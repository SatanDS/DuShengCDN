...
- 节点列表与节点详情的运行态文案继续统一为 `代理服务`：`OpenResty 异常`、`OpenResty 健康与控制`、`OpenResty 吞吐`、`重启 OpenResty` 等用户可见文案改成 `代理服务异常`、`代理服务健康与控制`、`代理服务吞吐`、`重启代理服务`；版本字段和底层配置字段仍保留技术名。
- 代理服务配置页和观测计量继续统一：运行调优、反代缓冲、压缩、缓存、主配置模板、配置预览、带宽趋势等标题与说明改为 `代理服务`；底层参数名、`nginx.conf` 和 `OpenResty*` Option 键仍保留，避免影响保存和渲染链路。
- 代理服务配置页的表单标签继续新手化：`worker_processes`、`proxy_connect_timeout`、`gzip_min_length`、`proxy_cache_path` 等底层指令名不再直接作为主标签，改为 `工作进程数量`、`源站连接超时`、`压缩最小响应大小`、`缓存目录` 等白话中文；底层指令名保留在 `!` 说明里。
- 网站详情、节点编辑、配置导航、发布预览和总览页继续统一：`自动 DNS` 改为 `自动解析域名` 或 `自动解析`，`OpenResty 参数变化/入站/出站/异常/状态` 改为 `代理服务参数变化/入站/出站/异常/状态`；自动解析里的连接数和内存上限也改成更直观的 `最大连接数`、`最大内存压力`。
- 发布预览、版本快照、应用日志和节点详情继续白话化：`Checksum` 改为 `校验值`，`Node ID` 改为 `节点 ID`，`Target Checksum` 改为 `目标校验值`，英文主配置标题改为中文；节点详情里的 `Agent` 和 `Nginx` 展示名改为 `节点程序版本`、`代理服务内核`。
- 防护、总览和接入设置继续白话化：`PoW 防护` 改为 `计算验证防护`，`WAF 防护` 改为 `恶意请求防护`，`QPS/CPU` 在主展示中改为 `每秒请求/处理器压力`，设置页 `Discovery Token/Server URL` 改为 `接入令牌/面板访问地址`。
...
4. 如果网站自动解析不能解析到本地自建解析，检查网站详情的 `解析模式` 是否为 `本地自建解析`，托管域名是否匹配，在线 DNS 响应端是否有未过期且一致的解析配置。
