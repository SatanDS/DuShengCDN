...
* 管理端能在「权威 DNS」迁移向导里列出 Cloudflare 模式站点候选，检查 Zone、在线 Worker、公网 UDP/TCP 53 探测、同名静态记录冲突、global 和显式来源作用域的可用边缘 IP、GSLB 与回滚提示，并对满足条件的站点执行一键切换；网站详情手动保存启用的权威 DNS 站点时同样阻断无法返回边缘 IP 的配置。
