...
| 一键安装 DNS Worker 数据目录 | `/opt/dushengcdn-dns-worker/data` | 安装脚本默认生成，用于保存快照缓存、来源库元数据、可选 Country/ASN MMDB 和运营商 CIDR |
...
| `DUSHENGCDN_LISTEN_ADDRESS` / `LISTEN_ADDRESS` | Server 监听地址；systemd 商业安装默认写入 `127.0.0.1`，需要公网直连时才显式改成 `0.0.0.0` 并配套防火墙/HTTPS 反代 | 空，监听所有地址 |
...
| `DUSHENGCDN_PUBLIC_STATUS_RUNTIME_METADATA` | 是否允许未登录 `/api/status` 返回运行版本、启动时间和 `ServerAddress`；默认关闭以减少公网指纹信息 | `false` |
| `AGENT_TOKEN` | 兼容旧部署的全局 Agent Token；未开启 `DUSHENGCDN_AGENT_LEGACY_GLOBAL_TOKEN_ENABLED=true` 时不会接受 | 空 |
| `DUSHENGCDN_AGENT_LEGACY_GLOBAL_TOKEN_ENABLED` | 迁移旧 Agent 时临时允许全局 `AGENT_TOKEN` 认证 | `false` |
...
| `DUSHENGCDN_LICENSE_ISSUER_PRIVATE_KEY_FILE` | 开发者授权面板读取 Ed25519 签发私钥的文件路径；推荐，仅应配置在签发端 | 空 |
| `DUSHENGCDN_LICENSE_ISSUER_PRIVATE_KEY` | 开发者授权面板使用的 Ed25519 签发私钥，支持 base64url、标准 base64 或 hex；兼容变量，避免在 Compose/.env、shell 历史或共享环境中使用 | 空 |
| `DUSHENGCDN_DNSSEC_KEY_ENCRYPTION_KEY` | DNSSEC 私钥加密密钥；启用权威 DNS DNSSEC 前必须配置，并且 Server 与所有 DNS Worker 必须使用同一个值 | 空 |
| `DUSHENGCDN_SERVER_AUTO_UPGRADE_ENABLED` | 是否允许管理端从 GitHub Release 自动下载并替换 Server 二进制 | `false` |
| `DUSHENGCDN_SERVER_UPDATE_REPO` | 顶栏“版本”检查使用的 GitHub Release 仓库，格式 `owner/repo`；私库商业版建议指向只放二进制的发布仓库 | `SatanDS/SatanDS-DuShengCDN-releases` |
...
* 旧全局 `AGENT_TOKEN` 兼容默认关闭。只有迁移旧版 Agent 时才临时设置 `DUSHENGCDN_AGENT_LEGACY_GLOBAL_TOKEN_ENABLED=true` 或运行时 Option `AgentLegacyGlobalTokenEnabled=true`；新部署应使用 Discovery Token 首次注册，或使用节点详情里的专属 `agent_token`。旧全局 Token 请求必须携带已存在的 `node_id`，且不能覆盖已经切换为专属 Token 的节点。
...
* 开发者签发面板需要额外配置 `DUSHENGCDN_LICENSE_ISSUER_PRIVATE_KEY_FILE`，或兼容变量 `DUSHENGCDN_LICENSE_ISSUER_PRIVATE_KEY`。该私钥只用于生成许可证，不应配置在客户生产部署中；客户部署只需要 `DUSHENGCDN_LICENSE_PUBLIC_KEYS`。
* `DUSHENGCDN_DNSSEC_KEY_ENCRYPTION_KEY` 建议使用 `openssl rand -base64 48` 生成，并同时写入 Server 环境和所有 DNS Worker 的 `dns-worker.env`。启用 DNSSEC 后不要随意轮换该值，否则已生成的 DNSSEC 私钥无法解密，需要重新生成密钥并重新提交注册商 DS。
...
* 商业-only 部署不要把源码仓库读权限交给客户，也不要把 GitHub token 下发到 Agent。推荐源码仓库保持私有，另建只放 Release 资产的二进制仓库，例如 `SatanDS/SatanDS-DuShengCDN-releases`，让 `DUSHENGCDN_SERVER_UPDATE_REPO` 和 `AgentUpdateRepo` 指向它。该二进制仓库可以公开下载但不含源码，运行权限由商业授权令牌控制；如一定要私有下载，Server 顶栏检查可配置仅能读取二进制仓库的 `DUSHENGCDN_GITHUB_RELEASE_TOKEN`，Agent 自更新则应关闭或后续改为 Server 代理/短期签名 URL，不应直接给 Agent GitHub token。
...
# 把 keygen 输出的 private_key 保存到仓库外、权限受限的文件中，例如：
...
`keygen` 输出的 `public_key` 配置到 `DUSHENGCDN_LICENSE_PUBLIC_KEYS`；`private_key` 只用于签发端，应离线保存，并通过 `-private-key-file` 从权限受限文件读取，不要放进 shell 历史、进程参数、Server 环境变量或 Compose 文件。`features` 可使用 `all`，或组合 `acme-automation`、`authoritative-dns`、`cloudflare-dns`、`gslb`、`ddos-protection`、`waf`、`cc-protection`、`geo-access-control`。

如需在管理端直接签发客户授权，可把 `keygen` 输出的 `private_key` 保存到仓库外、权限受限的文件，并把文件路径配置到专用签发面板所在的 Server：
...
| `AgentLegacyGlobalTokenEnabled` | 迁移旧 Agent 时临时允许全局 `AGENT_TOKEN`；生产迁移完成后应关闭 | `false` |
...
| `AgentUpdateRepo` | Agent 自更新仓库；私库商业版建议指向只放二进制的发布仓库 | `SatanDS/SatanDS-DuShengCDN-releases` |
...
* `AgentUpdateRepo` 指向的 GitHub Release 必须为每个 Agent 二进制提供同名 `.sha256` 校验文件和 `.sig` 签名文件，例如 `dushengcdn-agent-linux-amd64.sha256` 与 `dushengcdn-agent-linux-amd64.sig`；Agent 自更新会在替换可执行文件前校验 SHA-256 与 Ed25519 签名。
...
| `NEXT_PUBLIC_APP_VERSION` | 前端静态构建版本号；未登录 `/api/status` 默认不公开运行版本，只有开启 `DUSHENGCDN_PUBLIC_STATUS_RUNTIME_METADATA=true` 时才返回 Server 版本 | `dev` |
...
| `--dns-worker-source-database-profile` | DNS Worker 来源库预设：`full`、`country`、`asn`、`operator`、`none` | `full` |
| `--dns-worker-no-source-database-download` | 不自动下载 Country/ASN/运营商来源库 | `false` |
| `--dns-worker-no-source-database-update-timer` | 不创建 DNS Worker 7 天来源库更新 timer | `false` |
| `--dns-worker-no-geoip-download` | 兼容旧参数，等同于不自动下载来源库 | `false` |
...
| 变量 | 作用 | 默认值 |
| --- | --- | --- |
| `DUSHENGCDN_GO_VERSION` | 源码构建兜底时自动安装的 Go 版本 | `1.26.4` |
...
| `DUSHENGCDN_AGENT_TOKEN_FILE` | 从文件读取节点专属认证 Token，可覆盖 `agent.json`，推荐 | 空 |
| `DUSHENGCDN_DISCOVERY_TOKEN_FILE` | 从文件读取首次自动注册 Token，可覆盖 `agent.json`，推荐 | 空 |
| `DUSHENGCDN_AGENT_TOKEN` | 节点专属认证 Token，可覆盖 `agent.json`，兼容变量，避免在共享环境中使用 | 空 |
| `DUSHENGCDN_DISCOVERY_TOKEN` | 首次自动注册 Token，可覆盖 `agent.json`，兼容变量，避免在共享环境中使用 | 空 |
...
| `DUSHENGCDN_GEOIP_LOOKUP_API_TOKEN_FILE` | 从文件读取可选在线 IP 精确查询 API Bearer Token，可覆盖 `agent.json`，推荐 | 空 |
| `DUSHENGCDN_GEOIP_LOOKUP_API_TOKEN` | 可选在线 IP 精确查询 API Bearer Token，可覆盖 `agent.json`，兼容变量，避免在共享环境中使用 | 空 |
...
| `DUSHENGCDN_DNS_WORKER_ID` | DNS Worker 在面板中的响应端 ID；用于 Agent 代执行更新前确认本机实例身份 | 空 |
| `DUSHENGCDN_DNS_WORKER_TOKEN_FILE` | 从文件读取 DNS Worker 专属认证 Token，推荐 | 空 |
| `DUSHENGCDN_DNS_WORKER_TOKEN` | DNS Worker 专属认证 Token，兼容变量，避免在共享环境中使用 | 空 |
...
| `DUSHENGCDN_DNSSEC_KEY_ENCRYPTION_KEY` | DNSSEC 私钥加密密钥；启用权威 DNS DNSSEC 前必须配置，并且 DNS Worker 必须和 Server 使用同一个值 | 空 |
| `DUSHENGCDN_DNS_WORKER_SOURCE_DATABASE_PROFILE` | 安装脚本来源库预设：`full`、`country`、`asn`、`operator`、`none` | `full` |
| `DUSHENGCDN_DNS_WORKER_SOURCE_DATABASE_UPDATE_TIMER` | 安装脚本是否创建 7 天来源库更新 timer | `true` |
| `DUSHENGCDN_DNS_WORKER_GEOIP_DATABASE_PATH` | 可选本地 MaxMind Country/City/Enterprise MMDB 路径，用于按国家代码匹配 GSLB 节点池 | 空 |
| `DUSHENGCDN_DNS_WORKER_ASN_DATABASE_PATH` | 可选本地 MaxMind ASN MMDB 路径，用于按 ASN 匹配 GSLB 节点池 | 空 |
| `DUSHENGCDN_DNS_WORKER_OPERATOR_CIDR_DATABASE_PATH` | 可选本地 gaoyifan/china-operator-ip CIDR 目录或文件，用于按中国运营商匹配 GSLB 节点池 | 空 |
| `DUSHENGCDN_DNS_WORKER_GEOIP_DATABASE_URL` | 安装脚本 Country MMDB 下载地址 | Loyalsoldier GeoLite2-Country |
| `DUSHENGCDN_DNS_WORKER_ASN_DATABASE_URL` | 安装脚本 ASN MMDB 下载地址 | Loyalsoldier GeoLite2-ASN |
| `DUSHENGCDN_DNS_WORKER_OPERATOR_CIDR_BASE_URL` | 安装脚本运营商 CIDR 下载基址 | gaoyifan/china-operator-ip `ip-lists` |
| `DUSHENGCDN_DNS_WORKER_OPERATOR_CIDR_FILES` | 安装脚本下载的运营商 CIDR 文件列表，空格分隔 | 常用中国运营商 IPv4/IPv6 列表 |
| `DUSHENGCDN_DNS_SOURCE_DATABASE_MIRROR_PATH` | 面板服务器端来源库镜像目录；为空时使用上传目录下 `dns-source-databases` | 空 |
| `DUSHENGCDN_DNS_SOURCE_DATABASE_COUNTRY_URL` | 面板来源库镜像 Country MMDB 下载地址 | Loyalsoldier GeoLite2-Country |
| `DUSHENGCDN_DNS_SOURCE_DATABASE_ASN_URL` | 面板来源库镜像 ASN MMDB 下载地址 | Loyalsoldier GeoLite2-ASN |
| `DUSHENGCDN_DNS_SOURCE_DATABASE_OPERATOR_CIDR_BASE_URL` | 面板来源库镜像运营商 CIDR 下载基址 | gaoyifan/china-operator-ip `ip-lists` |
| `DUSHENGCDN_DNS_SOURCE_DATABASE_OPERATOR_CIDR_FILES` | 面板来源库镜像运营商 CIDR 文件列表，空格分隔 | 常用中国运营商 IPv4/IPv6 列表 |

DNS Worker 心跳会把本地国家、ASN、运营商来源库状态同步到 Server，包括数据库路径、数据库类型和最近加载错误。管理端会据此展示对应节点池是否具备命中条件；某类来源库未加载时，对应国家/ASN/运营商节点池不会命中，来源 CIDR 与 `global` 调度仍可继续工作。响应端安装脚本会优先从 GitHub 下载来源库，失败时请求面板镜像；周期更新时只有面板镜像比本地新才会回退替换。面板镜像可在设置页手动刷新，也会每 7 天自动刷新并删除上一份备份。

## DNS Worker 命令行参数

一键安装脚本 `scripts/install-dns-worker.sh` 会把这些字段写入 `/opt/dushengcdn-dns-worker/dns-worker.env`，并创建 `dushengcdn-dns-worker.service`。脚本参数与运行时环境变量保持同名语义，例如 `--server-url` 写入 `DUSHENGCDN_DNS_WORKER_SERVER_URL`，`--worker-id` 写入 `DUSHENGCDN_DNS_WORKER_ID` 供 Agent 代执行更新前校验本机响应端身份，推荐用 `--token-file` 从本地权限受限文件读取 Token；`--token` 默认拒绝，只有显式传 `--allow-insecure-token-argv` 时才作为兼容入口接受。脚本会把运行时 Token 持久化到权限受限的 token 文件，并在环境文件中写入 `DUSHENGCDN_DNS_WORKER_TOKEN_FILE`。脚本默认会把 Country MMDB 下载到 `/opt/dushengcdn-dns-worker/data/geoip/GeoLite2-Country.mmdb` 并写入 `DUSHENGCDN_DNS_WORKER_GEOIP_DATABASE_PATH`；可用 `--geoip-database-url` 指定下载源，或用 `--no-geoip-download` 关闭自动下载。

| 参数 | 作用 | 默认值 |
| --- | --- | --- |
| `--server-url` | DuShengCDN Server 地址 | 环境变量 |
| `--worker-id` | DNS Worker 在面板中的响应端 ID，用于绑定 Agent 代执行更新 | 环境变量 |
| `--token-file` | 从文件读取 DNS Worker 专属 Token | 环境变量 |
| `--token` | DNS Worker 专属 Token，兼容参数；默认拒绝，只有显式传 `--allow-insecure-token-argv` 时才接受 | 环境变量 |
...
| `--source-database-profile` | 来源库预设：`full`、`country`、`asn`、`operator`、`none` | `full` |
| `--geoip-database` | 可选本地 MaxMind Country/City/Enterprise MMDB 路径 | 空 |
| `--geoip-database-url` | Country MMDB 下载地址 | Loyalsoldier GeoLite2-Country |
| `--asn-database` | 可选本地 MaxMind ASN MMDB 路径 | 空 |
| `--asn-database-url` | ASN MMDB 下载地址 | Loyalsoldier GeoLite2-ASN |
| `--operator-cidr-database` | 可选本地 gaoyifan/china-operator-ip CIDR 目录或文件 | 空 |
| `--operator-cidr-base-url` | 运营商 CIDR 下载基址 | gaoyifan/china-operator-ip `ip-lists` |
| `--operator-cidr-files` | 下载的运营商 CIDR 文件列表，空格分隔 | 常用中国运营商 IPv4/IPv6 列表 |
| `--no-source-database-download` | 不自动下载来源库 | `false` |
| `--no-source-database-update-timer` | 不创建 7 天来源库更新 timer | `false` |
...
| `openresty_observability_port` | 本地观测与 OpenResty 健康检查端口；仅应回环访问，不要公网映射 | 否 | `18081` |
...
| `geoip_lookup_api_token_file` | 从权限受限文件读取可选在线 IP 精确查询 API Bearer Token，推荐 | 否 | 空 |
| `geoip_lookup_api_token` | 可选在线 IP 精确查询 API Bearer Token，兼容旧配置，不建议持久化明文 | 否 | 空 |
...
* Agent 周期性健康检查会请求 `http://127.0.0.1:<openresty_observability_port>/dushengcdn/stub_status`，不再通过高频 `openresty -t` 判断运行时健康；配置应用、启动恢复和 reload 前校验仍会执行 `openresty -t -c <main_config_path>`。该端口只用于本机观测，Docker 端口映射和主机防火墙必须限制在回环地址。
