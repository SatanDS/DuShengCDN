...
| `agent_token` | 左侧「节点和IP池」-> 新增或选择节点 ->「详情」->「节点信息」->「节点标识与部署」 | 适合单节点预创建和定向接入 |
...
| `--discovery-token-file` | 从文件读取首次自动注册 Token，推荐 |
| `--agent-token-file` | 从文件读取节点专属 Token，推荐 |
...
| `--geoip-api-token-file` | 从权限受限文件读取在线 IP 精确查询 API Bearer Token，推荐 |
| `--geoip-api-token` | 可选的在线 IP 精确查询 API Bearer Token，兼容旧脚本，不建议使用 |
...
`openresty_observability_port` 只用于 Agent 本机健康检查和 `stub_status`，必须保持在回环地址访问范围内；Docker 或防火墙不要把 `18081` 暴露到公网。
