...
你会学到：如何为 DuShengCDN 配置 GitHub OAuth 或标准 OIDC 登录入口，如何填写回调地址，以及第三方账号如何绑定本地用户。

DuShengCDN 支持通过认证源配置第三方登录入口。当前支持 GitHub OAuth 与标准 OIDC Provider，例如 Logto、authentik、Keycloak、Casdoor 等。
...
| 项目 | 说明 |
| --- | --- |
| DuShengCDN 访问地址 | 用户浏览器实际访问的地址，例如 `https://dushengcdn.example.com` |
| 认证源名称 | DuShengCDN 内部唯一标识，例如 `github`、`company-oidc` |
...
2. `Homepage URL` 填写 DuShengCDN 访问地址。
3. `Authorization callback URL` 填写 DuShengCDN 显示的回调地址，例如 `https://dushengcdn.example.com/oauth/github`。
4. 复制 GitHub 提供的 Client ID 和 Client Secret。
5. 登录 DuShengCDN 管理端，进入“设置 -> 系统设置 -> 配置认证源”。
6. 新增认证源，类型选择 `GitHub`。
...
3. Redirect URI / Callback URL 填写 DuShengCDN 显示的回调地址，例如 `https://dushengcdn.example.com/oauth/company-oidc`。
4. 复制 Client ID 和 Client Secret。
5. 获取 Provider 的 Discovery URL，通常以 `/.well-known/openid-configuration` 结尾。
6. 登录 DuShengCDN 管理端，进入“设置 -> 系统设置 -> 配置认证源”。
7. 新增认证源，类型选择 `OIDC`。
...
检查第三方平台中配置的 Redirect URI / Callback URL 是否与 DuShengCDN 表单提示完全一致。协议、域名、端口和路径都必须一致。

### 登录页没有显示第三方登录按钮

检查认证源是否已启用，并确认 Client ID 和 Client Secret 已保存。启用认证源前，DuShengCDN 会校验这些字段。

### 已经保存 Client Secret，但列表不显示明文

这是预期行为。DuShengCDN 不会通过 API 回显 Client Secret，只显示该密钥是否已配置。
