...
| 端口 | Server 容器内默认 `3000`，源码 Compose 默认发布宿主机 `3010`；Agent 节点对外使用 `80` / `443` |
| 备份 | 升级前备份数据库和 Server 数据目录 |

如果宿主机本机端口 `3000` 已被占用，只改宿主机侧端口，例如 `3010:3000`；容器内部仍监听 `3000`。如需只允许本机 HTTPS 反向代理访问，可改成 `127.0.0.1:3010:3000`。
...
| 负载均衡 | Cloudflare 记录、节点自动选点、多节点智能解析、橙云模式 |
| 缓存策略 | URL / 后缀 / 路径前缀 / 路径包含 / 路径多片段 / 精确路径缓存规则 |
| CC 防护 | 访问频率限制、命中后拦截或转入计算验证、计算验证白名单/黑名单、恶意请求规则 |
| 地区限制 | 按国家或地区代码放行/拦截 |
| 认证配置 | 站点基础认证 |

CC 防护里的白名单、黑名单、排除名单或自定义拦截规则默认不展开；先选择要添加的规则类型，点击 `+` 后再填写匹配内容。若 CC 命中后选择转入计算验证，下方计算验证的算法、难度和规则会一起下发到节点。
...
路径中有动态数字或 ID 时，优先使用“按路径多片段缓存”。例如 Emby 封面路径 `/emby/Items/12039/Images` 中的 `12039` 会变化，可以添加两条规则 `/emby/Items/` 和 `/Images`，只有两个片段都出现在路径里才会缓存。
