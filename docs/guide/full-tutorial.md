...
| 字段 | 用途 |
| --- | --- |
| 节点池 | 自动 DNS 和缓存运行时操作的目标分组 |
| 公网 IP 池 | 可写入 A/AAAA 记录的节点公网地址 |
| 权重 | 加权调度时的优先级 |
| 参与自动调度 | 关闭后自动 DNS 跳过该节点 |
| 排空模式 | 开启后自动 DNS 和缓存操作都会跳过该节点 |
...
| 自动 DNS | Cloudflare 记录、节点自动选点、橙云模式 |
| 缓存策略 | URL / 后缀 / 路径缓存规则 |
| PoW 防护 | 反爬虫挑战、白名单、黑名单 |
| WAF 防护 | 观察/拦截模式、内置规则、白名单、自定义规则 |
| 地区限制 | 按国家或地区代码放行/拦截 |
| 认证配置 | 站点基础认证 |
...
左侧「节点/IP池」适合排查节点离线、OpenResty 异常、版本落后和健康事件。
