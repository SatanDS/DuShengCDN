...
生产环境建议在节点详情中维护节点池、公网 IP 池、调度权重和排空状态。负载均衡默认会按网站绑定的默认节点池选择在线且代理服务健康的公网 IP；启用多节点智能解析后，可在「负载均衡」配置中绑定多个节点池，按池权重、节点负载和防抖冷却时间同步 Cloudflare A/AAAA 记录。缓存清理、预热和攻击防护回退仍使用网站默认节点池内的在线节点。
