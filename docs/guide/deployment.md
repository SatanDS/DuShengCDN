...
Agent 安装脚本同样会优先下载 GitHub Release 里的二进制，并要求同名 `.sha256` 校验文件；如果匹配二进制缺少校验文件会中止，只有没有匹配二进制资产时才回退源码构建。源码构建会优先复用本机已有 Go；自动安装 Go 时会多源重试。网络环境对 `go.dev` 不稳定时，可在执行安装命令前设置 `DUSHENGCDN_GO_DOWNLOAD_BASE_URLS` 或完整归档地址 `DUSHENGCDN_GO_DOWNLOAD_URL`。
...
DNS Worker 使用安装脚本部署时，可重复执行安装命令进行重装或升级；脚本会优先下载当前仓库 Release 中的 DNS Worker 二进制并校验 `.sha256`。如果匹配二进制缺少校验文件会中止，只有没有匹配二进制资产时，脚本才会从源码构建并写入当前 Git 版本。卸载 DNS Worker：
...
* 安装脚本可重复执行，用于重装或升级 Agent；匹配 Release 二进制必须带同名 `.sha256`，没有匹配二进制资产时会从源码构建，并写入当前 Git 版本，避免显示为 `dev`。
