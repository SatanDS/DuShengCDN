...
启用 DNSSEC 前，Server 和所有 DNS Worker 必须配置同一个 `DUSHENGCDN_DNSSEC_KEY_ENCRYPTION_KEY`。该值用于加密数据库中的 DNSSEC 私钥，也用于 Worker 解密快照里的私钥并生成 DNSKEY/RRSIG；请保存到密码管理器或备份系统，启用后不要随意更换，否则已有 DNSSEC 私钥无法解密，需要重新生成密钥并重新提交 DS。

源码 Compose 同机部署可以这样配置：
...
面板启用 DNSSEC 后，再复制 DS 记录到注册商。关闭 DNSSEC 时先删除注册商 DS，等待缓存过期后再在面板关闭，避免外部验证解析失败。
