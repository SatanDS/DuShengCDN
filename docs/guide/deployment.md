...
4. 打开「权威 DNS」的「迁移向导」，确认待迁移网站已经匹配到启用 Zone、存在在线 Worker、至少一个 Worker 通过公网 UDP/TCP 53 探测且持有未过期调度快照，并按需启用站点 GSLB。
