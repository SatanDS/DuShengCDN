...
6. 回到「权威 DNS」使用「GSLB 调度模拟」按 HK、EU、global 等来源作用域预演返回目标，再到 Zone 详情执行「委派检查」，确认公网 NS 与期望 NS 匹配。
