...
* DNS Worker 会在每次 A/AAAA 查询时根据来源 IP/ECS、来源 CIDR、国家代码、来源分流桶、节点池权重、节点健康和负载评分返回边缘 IP。切换到自建权威 DNS 前，控制面会先复用同一组选点规则确认 global 和策略里声明的来源国家、来源 CIDR 至少有一个可返回目标；如果节点池名写错、节点无公网 IP、A/AAAA 类型不匹配、节点离线、OpenResty 不健康、排空或负载阈值过严，迁移向导和一键切换都会阻断。
