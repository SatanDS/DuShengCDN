...
* DNS Worker 会在每次 A/AAAA 查询时根据来源 IP/ECS、来源 CIDR、国家代码、来源分流桶、节点池权重、节点健康和负载评分返回边缘 IP。`load_aware` 查询优先在带有新鲜指标的节点中选择；当匹配节点池里暂无新鲜指标候选时，才回退到无指标节点，避免节点漏报指标时被误判为空闲。设置页「权威 DNS 运行参数」里的「启用 Agent 探测调度门槛」默认关闭；开启后，自建权威 DNS 快照和 DNS Worker 实时调度只会使用仍有新鲜成功 DNS Worker 探测结果的边缘节点，并在权重、负载评分等主规则相同的候选之间优先选择 Agent 多点探测平均 RTT 更低的节点。切换到自建权威 DNS 前，控制面会先复用同一组选点规则确认 global 和策略里声明的来源国家、来源 CIDR 至少有一个可返回目标；如果节点池名写错、节点无公网 IP、A/AAAA 类型不匹配、节点离线、OpenResty 不健康、排空、负载阈值过严，或开启探测调度门槛后节点缺少新鲜成功探测，迁移向导、一键切换和网站详情启用保存都会阻断。
...
* Worker 延迟来自 DNS Worker 本地处理真实查询的聚合耗时；Agent 多节点探测 RTT 则表示各边缘节点到 Worker NS 的主动探测耗时。超过新鲜度窗口的 Agent 多点探测会标记为“探测过期”，仍保留明细但不计入健康通过率；「GSLB 调度模拟」里的 Agent 探测摘要用于解释边缘节点到各 NS 的可达性。默认情况下多点探测只用于观测与排障；只有开启「启用 Agent 探测调度门槛」后，才会影响自建权威 DNS 的 GSLB 选点，其中平均 RTT 只作为同等健康候选之间的辅助排序依据。
