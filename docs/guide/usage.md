...
* DNS Worker 会在每次 A/AAAA 查询时根据来源 IP/ECS、来源 CIDR、国家代码、来源分流桶、节点池权重、节点健康和负载评分返回边缘 IP。`load_aware` 查询优先在带有新鲜指标的节点中选择；当匹配节点池里暂无新鲜指标候选时，才回退到无指标节点，避免节点漏报指标时被误判为空闲。切换到自建权威 DNS 前，控制面会先复用同一组选点规则确认 global 和策略里声明的来源国家、来源 CIDR 至少有一个可返回目标；如果节点池名写错、节点无公网 IP、A/AAAA 类型不匹配、节点离线、OpenResty 不健康、排空或负载阈值过严，迁移向导、一键切换和网站详情启用保存都会阻断。
...
* 左侧「权威 DNS」的「GSLB 调度模拟」可选择权威 DNS 站点、记录类型、来源国家代码和来源 IP，预演当前快照会返回哪些边缘 IP、TTL 和来源作用域，并查看节点池匹配、候选节点、跳过节点、负载指标时间和原因；该操作只读，不会写入真实防抖状态。
