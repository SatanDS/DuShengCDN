...
* 左侧「权威 DNS」会展示最近 24 小时的查询量、查询趋势、SERVFAIL/NXDOMAIN 趋势、Worker 快照一致性、Worker 查询延迟、可用率、错误率、最近公网探测健康状态、Worker/Zone/站点维度和返回目标分布，适合确认 GSLB 是否按预期把查询分配到 HK、EU 等节点池，并发现多 Worker 快照版本不一致或快照过期问题。
