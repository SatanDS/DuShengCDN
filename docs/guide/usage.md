...
* GSLB 的 `负载感知` 模式会结合节点权重、池权重、OpenResty 当前连接数、CPU 和内存快照评分；在同一区域可配置最大连接数、最大 CPU 使用率和最大内存使用率，超过任一阈值的节点会被跳过。
...
* 左侧「权威 DNS」的「迁移向导」会列出 Cloudflare 模式网站候选，检查是否已有匹配 Zone、在线 Worker、公网 UDP/TCP 53 探测、未过期 Worker 调度快照、同名静态记录冲突、当前节点池或 GSLB 策略是否能返回可用边缘 IP，以及策略里显式配置的来源国家、来源 CIDR 是否都有可用边缘 IP；满足条件时可直接点击「一键切换」，也可以跳转到网站详情手动调整。一键切换成功后，向导会自动刷新网站 DNS 模式、执行 Zone 委派检查、探测在线 Worker，并按当前快照执行 global 与来源国家 GSLB 模拟复测。
* 网站详情的「自动 DNS」里仍可把 `DNS 模式` 从 `Cloudflare 同步` 切换为 `自建权威 DNS`，再绑定已启用的 DNS Zone；启用状态下保存时会执行和迁移向导一致的 DNS Worker 在线、公网探测、快照就绪、可用边缘 IP 和显式来源作用域预检，未启用草稿可先保存再补齐节点与 Worker。
...
* DNS Worker 会在每次 A/AAAA 查询时根据来源 IP/ECS、来源 CIDR、国家代码、来源分流桶、节点池权重、节点健康和负载评分返回边缘 IP。切换到自建权威 DNS 前，控制面会先复用同一组选点规则确认 global 和策略里声明的来源国家、来源 CIDR 至少有一个可返回目标；如果节点池名写错、节点无公网 IP、A/AAAA 类型不匹配、节点离线、OpenResty 不健康、排空或负载阈值过严，迁移向导、一键切换和网站详情启用保存都会阻断。
...
* 迁移向导、一键切换和网站详情启用保存都会要求至少一个在线 DNS Worker 同时满足最新公网 UDP/TCP 53 探测通过、已拉取未过期调度快照，避免 Worker 只会连回 Server、但公网递归 DNS 无法访问 `53` 端口，或 Worker 尚未拿到可回答动态记录的快照时误切换或误保存。
