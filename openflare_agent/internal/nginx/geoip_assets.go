...
local ip_cache = ngx.shared.openflare_geoip_cache
local region_config_dict = ngx.shared.openflare_region_config
local db_path = "__OPENFLARE_GEOIP_DATABASE_PATH__"
local api_url = __OPENFLARE_GEOIP_LOOKUP_API_URL__
local api_token = __OPENFLARE_GEOIP_LOOKUP_API_TOKEN__
local api_timeout = __OPENFLARE_GEOIP_LOOKUP_API_TIMEOUT__
...
local function lookup_country_with_api(ip)
    ...
    local headers = {
        ...
        ["User-Agent"] = "OpenFlare-Agent-GeoIP",
    }
...
local function load_region_config()
    ...
    local config_paths = {
        "__OPENFLARE_RUNTIME_CONFIG_DIR__/region_config.json",
        "/etc/nginx/openflare-lua/region_config.json",
