...
local pow_config_dict = ngx.shared.dushengcdn_pow_config
local pow_sessions = ngx.shared.dushengcdn_pow_sessions

local function session_cookie(value, ttl)
    local cookie = "__dushengcdn_pow=" .. value .. "; Path=/; HttpOnly; SameSite=Lax; Max-Age=" .. tostring(ttl)
...
local function load_pow_config()
    local config_paths = {
        "__DUSHENGCDN_RUNTIME_CONFIG_DIR__/pow_config.json",
        "/etc/nginx/dushengcdn-lua/pow_config.json",
...
local cookie_val = ngx.var["cookie___dushengcdn_pow"]
...
local pow_config_dict = ngx.shared.dushengcdn_pow_config
local pow_challenges = ngx.shared.dushengcdn_pow_challenges
...
<script id="anubis_version" type="application/json">"dushengcdn-pow"</script>
...
<script id="anubis_public_url" type="application/json">"__dushengcdn_internal__"</script>
</head>
<body id="top">
<main>
<h1 id="title" class="centered-div">]] .. title .. [[</h1>
<div class="centered-div">
<img id="image" style="width:100%;max-width:256px;" src="]] .. static_prefix .. [[img/pensive.webp?cacheBuster=dushengcdn-pow">
...
<p>DuShengCDN is asking your browser to complete a lightweight computation to distinguish normal browser traffic from automated abuse. This should finish automatically.</p>
</details>
...
local cookie = "__dushengcdn_pow=" .. session_token .. "; Path=/; HttpOnly; SameSite=Lax; Max-Age=" .. tostring(session_ttl)
if ngx.var.scheme == "https" then
    cookie = cookie .. "; Secure"
end
...
function M.match_any(remote_ip, ua, uri, list)
    ...
    if ipmatcher.match_ips(remote_ip, list.ips) then return true end
    if ipmatcher.match_cidrs(remote_ip, list.ip_cidrs) then return true end
