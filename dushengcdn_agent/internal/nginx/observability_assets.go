...
const openRestyObservabilityInitLua = `local dict = ngx.shared.dushengcdn_observability
if not dict then
    return
end
...
const openRestyObservabilityLogLua = `local dict = ngx.shared.dushengcdn_observability
if not dict then
    return
end

local request_uri = tostring(ngx.var.uri or "")
if request_uri == "/dushengcdn/observability" or request_uri == "/dushengcdn/stub_status" then
    return
end
...
local dict = ngx.shared.dushengcdn_observability
if not dict then
